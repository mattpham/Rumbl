defmodule RumblWeb.AuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      # bypass_through passes router and :browser pipeline to invoke,
      # but bypasses route dispatch
  ...
  test "login with a not found user", %{conn: conn} do
    assert {:error, :not_found, conn} = Auth.login_by_email_and_pass(conn, "me@test", "secret")
  end

  test "login with password mismatch", %{conn: conn} do
    _ = user_fixture(username: "me", email: "me@test", password: "secret")

    assert {:error, :unauthorized, conn} = Auth.login_by_email_and_pass(conn, "me@test", "wrong")
  end
end
